...
static int transport_mgmt_connect(struct smp_streamer *ctxt)
{
	...
	if (outgoing_transport->functions.bridge_connect(&bridges[i], true,
			 (outgoing_transport == ctxt->smpt ? true : false), zsd, zse) == false) {
		transport_mgmt_unlock();
		goto end;
	}

	if (ctxt->smpt->functions.bridge_connect(&bridges[i], false,
			(outgoing_transport == ctxt->smpt ? true : false), zsd, zse) == false) {
...
LOG_DBG("Bridge %p to %p with %d", ctxt->smpt, outgoing_transport, i);
