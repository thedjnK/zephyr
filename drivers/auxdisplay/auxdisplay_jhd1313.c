...
static uint8_t colour_define[][4] = {
	{ 0,   0,   0   },      /* Off */
	{ 255, 255, 255 },	/* White */
	{ 255, 0,   0   },      /* Red */
	{ 0,   255, 0   },      /* Green */
...
static int auxdisplay_jhd1313_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
...
#define AUXDISPLAY_JHD1313_DEVICE(inst)							\
	static struct auxdisplay_jhd1313_config auxdisplay_jhd1313_config_##inst = {	\
		.capabilities = {							\
			.columns = 2,							\
			.rows = 16,							\
			.mode = 0,							\
	...
	PM_DEVICE_DT_INST_DEFINE(inst, auxdisplay_jhd1313_pm_action);			\
