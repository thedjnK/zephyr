...
static int auxdisplay_hd44780_pm_action(const struct device *dev, enum pm_device_action action);
#endif
...
#define AUXDISPLAY_HD44780_DEVICE(inst)								\
	...
	static const struct auxdisplay_hd44780_config auxdisplay_hd44780_config_##inst = {	\
		...
		.enable_line_rise_delay = DT_INST_PROP(inst, enable_line_rise_delay),		\
		.enable_line_fall_delay = DT_INST_PROP(inst, enable_line_fall_delay),		\
		.clear_delay = DT_INST_PROP(inst, clear_command_delay),				\
		.boot_delay = DT_INST_PROP(inst, boot_delay),					\
	};											\
