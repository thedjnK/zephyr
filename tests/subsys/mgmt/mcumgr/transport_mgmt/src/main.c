...
static bool transport_mgmt_config_decode(zcbor_state_t *state, struct transport_mgmt_configs *result)
{
	bool ok = true;
	size_t decoded;
	uint8_t entries = 0;
...
ZTEST(transport_mgmt, test_list)
{
	...
	zassert_equal(entries, 2, "Expected number of entires mismatch'");
...
ZTEST(transport_mgmt, test_valid_config_details)
{
	...
	zassert_equal(entries, 0, "Expected number of entires mismatch'");
