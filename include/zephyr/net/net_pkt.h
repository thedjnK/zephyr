...
struct net_pkt {
	...
	union {
#if defined(CONFIG_NET_IPV4) && defined(CONFIG_NET_IPV4_FRAGMENT)
		struct {
			uint16_t flags;		/* Fragment offset and M (More Fragment) flag */
			uint16_t id;		/* Fragment ID */
		} ipv4_fragment;
#endif /* CONFIG_NET_IPV4 && CONFIG_NET_IPV4_FRAGMENT */
#if defined(CONFIG_NET_IPV6) && defined(CONFIG_NET_IPV6_FRAGMENT)
		struct {
			uint16_t flags;		/* Fragment offset and M (More Fragment) flag */
